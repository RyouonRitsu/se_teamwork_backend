...
def register(request):
    if request.method == 'POST':
        ...
        if len(str(username)) == 0 or len(str(password_1)) == 0 or len(str(password_2)) == 0:
            return JsonResponse({'errno': 1011, 'msg': '必填字段為空'})
        try:
            _ = User.objects.get(username=username)
            return JsonResponse({'errno': 1004, 'msg': '用戶名已存在'})
        except User.DoesNotExist:
            if re.match('.*[^a-zA-Z\\d]+.*', str(username)) is not None:
                return JsonResponse({'errno': 1003, 'msg': '用戶名不合法'})
            elif password_1 != password_2:
                return JsonResponse({'errno': 1002, 'msg': '兩次輸入的密碼不同'})
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if author.password == password:
                ...
                return JsonResponse({'errno': 1002, 'msg': '密碼錯誤'})
        except User.DoesNotExist:
            return JsonResponse({'errno': 1006, 'msg': '用戶不存在'})
    else:
        return JsonResponse({'errno': 1001, 'msg': '請求方式錯誤'})
